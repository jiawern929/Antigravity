...
# Post details (Same article for the 3rd domain)
...
<p>但是，面对市场上琳琅满目的选择，反而让很多人不知所措。很多人会误以为，只要掏钱把最贵、最新的全能机型搬回家，就能解决所有的水质问题。其实这是一个非常普遍的消费迷思。我们在采访本地社区 of 资深水电师傅时，他们都异口同声地表示，挑选设备最重要的是因地制宜。如果你住的楼层水压本来就弱，却硬装上一台需要极高水压才能顺利运作的高密度净水器，最后受苦的只会是每天等着接水煮饭的自己。</p>
...
yoast_desc = "居住在巴生谷一带的你，是否经常面对自来水偏黄或带有漂白水异味的困扰？随着社区管线老化，越来越多家庭开始重视饮用水的二次污染问题。本文透过本地新闻视角的真实走访，为你全面剖析不同住宅类型的净水痛点，并提供最中立客观的KL滤水器推荐。无论你是住公寓还是有地房，都能从中找到避开隐藏开销、保障全家饮用水安全的专业核心结论与实用建议。"
...
def publish():
    ...
    try:
        ...
        # Step 5: Prompts the user for publishing approval inside the DOM (Bypasses Popup Blocker/Selenium restriction)
        print("Prompting user for publishing approval in the browser...")
        inject_banner(driver, "AI Step 5/5: WAITING FOR YOUR PERMISSION IN THE POPUP MODAL...")
        
        driver.execute_script("""
            var modal = document.createElement('div');
            modal.id = 'ai-approval-modal';
            modal.innerHTML = `
                <div style="
                    position: fixed;
                ...
                ">
                    <div style="
                        ...
                        text-align: center;
                    ">
                        <h3 style="margin-top: 0; font-size: 22px; font-weight: 700; color: #F8FAFC;">确认发布文章？</h3>
                        <p style="color: #94A3B8; font-size: 14px; line-height: 1.6; margin: 15px 0 25px 0;">
                            AI 已经准备好发布当前文章。<br>
                            请直接点击下方按钮确认或取消发布。
                        </p>
                        <div style="display: flex; gap: 12px; justify-content: center;">
                            <button id="ai-btn-cancel" style="
                                background: #334155;
                                color: #E2E8F0;
                                border: none;
                                padding: 10px 22px;
                                ...
                                transition: all 0.2s;
                            ">取消发布</button>
                            <button id="ai-btn-approve" style="
                                background: #2563EB;
                                color: white;
                                border: none;
                                padding: 10px 26px;
            ...
            window.ai_approved = null;
            document.getElementById('ai-btn-approve').onclick = function() {
                window.ai_approved = true;
                modal.remove();
            };
            document.getElementById('ai-btn-cancel').onclick = function() {
                window.ai_approved = false;
                modal.remove();
            };
        """)
        
        # Loop and poll for user click
        approved = None
        while True:
            try:
                approved = driver.execute_script("return window.ai_approved;")
        ...
        if not approved:
            ...
            inject_banner(driver, "ABORTED: Publishing cancelled by you. Keeping screen open.")
            while True:
                try:
        ...
        print("User approved! Publishing post via REST API...")
        inject_banner(driver, "AI: Permission granted! Submitting article and SEO data...")
        time.sleep(2)
        ...
        if result.get("success"):
            ...
            inject_banner(driver, "Done! The article is live. Screen will stay open, close it manually when done.")
        else:
            err_msg = result.get("error")
            print(f"\nFAILED to publish: {err_msg}")
            inject_banner(driver, f"Error publishing: {err_msg}")
            
        # Keep browser open indefinitely per user request
        while True:
            try:
                _ = driver.window_handles
            except Exception:
