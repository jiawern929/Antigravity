window.addEventListener("message", function(event){
  ...
  if (cmd === 'xtkt.processKeywords') {
  ...
  else if (cmd === 'xtkt.getCompetitorUrlKeywords') {
    chrome.runtime.sendMessage({
      cmd: 'api.getCompetitorUrlKeywords',
      data: data
    }, function(response){
      postResponse(cmd, handlerId, response);
    });
  ...
  else if (cmd === 'xtkt.getMatchingUrls') {
    chrome.runtime.sendMessage({
      cmd: 'api.getMatchingUrls',
      data: data
    }, function(response){
      postResponse(cmd, handlerId, response);
    });
    ...
    if (req.service === 'googleplay') {
      ...
      req = Object.assign({
        sourcePath: getParam('sourcePath'),
        hl: getParam('hl'),
