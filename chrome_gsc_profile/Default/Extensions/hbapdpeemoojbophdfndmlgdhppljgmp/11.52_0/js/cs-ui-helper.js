var UIHelper = (function(){
  ...
  var showIcon = function( type, content, popupRoot ){
    ...
    if (type === 'addKeywords') {
      ...
      $icon.attr('title', 'Add all keywords shown on the page to your favorite keyword list');
      // $icon.html('<span class="xt-star" data-state="off"></span> Add All Keywords');
      $icon.html(Common.getIcon('star'));
      addBrowserClass($icon);
    }
    else if (type === 'exportCSV') {
      $icon.addClass('xt-btn-exportCSV');
      $icon.attr('title', 'Export all keywords to a CSV file');
      $icon.html(Common.getIcon('export'));
  ...
  var initButtonsPosition = function($root){
    chrome.storage.local.get('buttonsPosition', function(data){
      ...
      var left = pos.left;
      var top = pos.top;
      if (left > document.body.clientWidth) left = document.body.clientWidth - 120;
      if (top > window.innerHeight) top = window.innerHeight - 50;
      if (left < 0) left = 0;
      if (top < 0) top = 0;
      var res = {
        left: left,
