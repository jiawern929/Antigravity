var Tool = (function(){
  ...
  function decodeBingUrl(href) {
    ...
    try {
      let b64 = decodeURIComponent(uMatch[1]);
      // Trim everything before aHR0 (base64 for "https")
      const index = b64.indexOf('aHR0');
      if (index > -1) b64 = b64.slice(index);
      return atob(b64);
    } catch (e) {
      console.warn('Failed to decode Bing obfuscated URL', e);
      return href;
    }
  }
  ...
  var processURLs = function(showGoogleTraffic, showGoogleMetrics){
    ...
    $items.map(function(i, node){
      ...
      // 1. Check for <data-sc-metadata> — Chrome and Edge may both have this
      var $scData = $parent.find('[data-sc-metadata]');
      if ($scData[0]) {
        try {
          var json = JSON.parse($scData[0].dataset.scMetadata);
          href = json.url;
        } catch (e) {
          console.log('Invalid JSON in scMetadata', e);
        }
      }
      // 2. Handle Bing obfuscated redirect links (e.g., Edge)
      if (/^https:\/\/www\.bing\.com\/ck\/a\?/.test(href)) {
