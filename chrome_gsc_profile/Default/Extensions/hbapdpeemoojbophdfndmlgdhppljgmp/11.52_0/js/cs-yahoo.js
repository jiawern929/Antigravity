var Tool = (function(){
  ...
  var initSuggestions = function(){
    var timer = setInterval(function(){
      if (!observer) {
        var node = $('.sa, .header-search-assist, form[role=search]')[0];
        console.log(node);
        if (node) {
          clearInterval(timer);
  ...
  var processSuggestion = function(node){
    ...
    $node.find('li[role=option]').map(function(i, item){
      ...
      var $item = $(item);
      if ($item.find('.xt-suggestions-search')[0]) return;
      if (!keyword) {
        keyword = $(item).find('.sa-item-title').text();
      }
      if (!keyword) {
        keyword = $.trim(item.textContent);
      }
      suggestionsList[keyword.toLowerCase()] = item;
    });
    if (!Object.keys(suggestionsList).length) return;
