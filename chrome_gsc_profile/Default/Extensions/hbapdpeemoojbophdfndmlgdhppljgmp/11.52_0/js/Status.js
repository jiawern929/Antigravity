...
(function(){
  ...
  Status.prototype.render = function() {
    ...
    if (this.type === 'success') bgClass = 'bg-green-500';
    var html = `
      <div class="mb-4 flex items-center rounded-lg ${bgClass} p-5 text-green-100 shadow-sm">
        <p class="content text-sm font-normal status-text">${this.html}</a>
        </p>
      </div>
    `;
    this.$root.html(html);
    var $elem = $(this.$root.children()[0]);
    $elem.hide();
    // show
