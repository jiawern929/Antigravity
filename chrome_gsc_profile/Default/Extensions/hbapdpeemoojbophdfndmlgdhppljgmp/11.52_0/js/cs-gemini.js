var Tool = (function(){
  ...
  var getSidebarInsertTarget = function(){
    var oldTarget = document.querySelector('side-nav-action-button[data-test-id="settings-and-help-button"]');
    if (oldTarget) return { target: $(oldTarget), method: 'before' };

    var desktopControls = document.querySelector('mat-action-list.desktop-controls');
    if (desktopControls) {
      var $controls = $(desktopControls);
      var $firstFooter = $controls.children('sidenav-mavatar-footer').first();
      if ($firstFooter.length) return { target: $firstFooter, method: 'before' };
      return { target: $controls, method: 'append' };
    }
  ...
  var startButtonRetry = function(){
    ...
    buttonRetryTimer = setInterval(function(){
      buttonRetryCount++;
      addWidgetButton();
      if (($('.xt-gemini-btn')[0] && $('.xt-gemini-btn-sec')[0]) || buttonRetryCount >= 40) {
  ...
  var addWidgetButton = function() {
    if (!$('.xt-gemini-btn')[0]) {
      darkMode = isDarkMode();
      let placement = getSidebarInsertTarget();
      if (placement && placement.target && placement.target.length) {
        let $btn = $(menuItemTemplate(darkMode));
        if (placement.method === 'append') $btn.appendTo(placement.target);
        else $btn.insertBefore(placement.target);
