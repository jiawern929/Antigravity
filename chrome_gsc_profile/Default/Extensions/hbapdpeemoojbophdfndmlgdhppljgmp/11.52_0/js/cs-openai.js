var Tool = (function(){
  ...
  var profileButtonSelector = '[data-testid="accounts-profile-button"], [data-testid="accounts-button"], [data-testid="profile-button"]';
  var mainProfileButtonSelector = 'main [data-testid="accounts-profile-button"], main [data-testid="accounts-button"], main [data-testid="profile-button"]';
  var chatHistoryNavSel = 'nav[aria-label="Chat history"]';


  var isProfileButtonTestId = function(testId){
    return testId === 'accounts-profile-button' || testId === 'accounts-button' || testId === 'profile-button';
  };


  var findProfileButton = function(root){
    root = root || document;
    return root.querySelector(profileButtonSelector);
  };


  var getChatHistoryNav = function(){
    var nav = document.querySelector(chatHistoryNavSel);
    if (nav) return nav;
    var navs = document.querySelectorAll('nav');
    for (var i = 0; i < navs.length; i++) {
      if (navs[i].getAttribute('aria-label') === 'Chat history') return navs[i];
      var cls = navs[i].className || '';
      if (typeof cls === 'string' && cls.indexOf('scrollport') !== -1) return navs[i];
    }
    return navs.length >= 2 ? navs[1] : null;
  };


  var getSidebarRoot = function(){
    var nav = getChatHistoryNav();
    if (!nav) return null;
    return nav.closest('[class*="sidebar"]') || nav.parentElement || nav;
  };


  var isInSidebarColumn = function(el){
    if (!el) return false;
    var nav = getChatHistoryNav();
  ...
  var findSidebarProfileButton = function(){
    var root = getSidebarRoot();
    if (!root) return null;
    var candidates = root.querySelectorAll(profileButtonSelector);
    var best = null;
    var bestTop = -1;
  ...
  /** Bottom-of-sidebar anchor when profile button is not matched (UI varies by account). */
  var findSidebarFooterAnchor = function(){
    var root = getSidebarRoot();
    if (!root) return null;
    var selectors = [
        ...
        if (mutation.type === 'childList') {
          ...
          for (var i = 0, len = mutation.addedNodes.length; i < len; i++) {
            ...
            if (node.nodeType === Node.ELEMENT_NODE) {
              if (node.matches && node.matches(chatHistoryNavSel)) {
                addWidgetButton();
              }
              if (node.querySelector && node.querySelector(chatHistoryNavSel)) {
                addWidgetButton();
              }
            }
            ...
            if (node.nodeType === Node.ELEMENT_NODE) {
              if (node.dataset && isProfileButtonTestId(node.dataset.testid)) {
                addWidgetButton();
              }
              if (node.querySelector && node.querySelector(profileButtonSelector)) {
                addWidgetButton();
              }
            }
          }
  ...
  var isVisibleElement = function(el){
  ...
  var getSidebarPlacement = function(){
    var chatNav = getChatHistoryNav();
    var profileBtn = findSidebarProfileButton();
    if (!chatNav) {
      return null;
    }
    if (profileBtn && isVisibleElement(profileBtn)) {
      return { target: $(profileBtn), method: 'before' };
    }

    var $legacyAside = $(chatNav).children('aside').first();
    if ($legacyAside.length && isVisibleElement($legacyAside[0])) {
      return { target: $legacyAside, method: 'append' };
    }

    var footerAnchor = findSidebarFooterAnchor();
    if (footerAnchor && isVisibleElement(footerAnchor)) {
      return { target: $(footerAnchor), method: 'before' };
    }

    return { target: $(chatNav), method: 'append' };
  };


  var ensureSidebarTemplatesButton = function(){
    var placement = getSidebarPlacement();
    if (!placement || !placement.target || !placement.target.length) {
      return;
    }

    var $btn = $('.xt-openai-templates-btn').first();
    if ($btn.length && (!isVisibleElement($btn[0]) || !isInSidebarColumn($btn[0]))) {
      $btn.remove();
      $btn = $();
    }

    if ($btn.length) {
      var anchor = placement.target[0];
      if (placement.method === 'before' && anchor) {
        if ($btn[0] !== anchor.previousElementSibling) {
          $btn.insertBefore(placement.target);
        }
      } else if (placement.method === 'append' && anchor) {
        if ($btn[0].parentElement !== anchor || $btn[0] !== anchor.lastElementChild) {
          $btn.appendTo(placement.target);
        }
      }
      return;
    }
    ...
    var $newBtn;
    if (placement.method === 'append') {
      $newBtn = $(menuItemTemplate(darkMode)).appendTo(placement.target);
    } else {
      $newBtn = $(menuItemTemplate(darkMode)).insertBefore(placement.target);
    }
    $newBtn.click(function(e) {
      toggleWidget();
    });
    addSidebarIframe($newBtn, placement.target);
  };
  ...
  var menuItemTemplate = function(darkmode, sec){
    ...
    return `
      <a tabindex="0" data-fill="" data-sidebar-item="true" class="${className} group __menu-item hoverable ms-2 me-1.5 gap-2! pe-1.5 data-fill:max-w-full [&>div:first-child]:gap-2!"><div class="flex min-w-0 items-center gap-2"><div class="flex items-center justify-center icon-lg icon ${ darkmode ? 'text-white' : ''}">${LOGO_SVG}</div><div class="min-w-0"><div class="flex min-w-0 grow items-center gap-2.5"><div class="truncate">Templates</div></div></div></div></a>
    `;
  };
  ...
  var addWidgetButton = function() {
    darkMode = isDarkMode();
    let sidebarPlacement = getSidebarPlacement();
    let $logoMenu = $( $('main [aria-haspopup="menu"]')[0] );
    let $loginBtn = $( $('main [data-testid="login-button"]')[0] );
    let $profileBtn = $( $(mainProfileButtonSelector)[0] );
    let $btn;
    let $secBtn;
    ensureSidebarTemplatesButton();
    if ($('.xt-openai-templates-btn')[0] && sidebarPlacement && sidebarPlacement.target && sidebarPlacement.target[0]) {
      addSidebarIframe($('.xt-openai-templates-btn').first(), sidebarPlacement.target);
    }
  ...
  var addSidebarIframe = function($btn, $modeButton){
    if ($('#xt-chatgpt-sidebar-iframe')[0]) {
      return;
    }
    ...
    if (profileBtn && profileBtn.parentNode) {
      profileBtn.parentNode.insertBefore($root[0], profileBtn);
      return;
    }
    var chatNav = getChatHistoryNav();
    var firstAside = chatNav ? chatNav.querySelector(':scope > aside, aside') : null;
    if (firstAside && firstAside.parentNode && isVisibleElement(firstAside)) {
      firstAside.parentNode.insertBefore($root[0], firstAside);
      return;
    }
    if ($modeButton && $modeButton[0]) {
