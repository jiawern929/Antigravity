var Common = (function(){
  ...
  var appendLTKBtn = function(html, params){
    ...
    if (params.sourcePath) btnURL += '&sourcePath=' + encodeURIComponent(params.sourcePath);
    if (params.hl) btnURL += '&hl=' + encodeURIComponent(params.hl);
    if (params.catalog) btnURL += '&catalog=' + encodeURIComponent(params.catalog);
    if (params.at) btnURL += '&at=' + encodeURIComponent(params.at);
    if (params.bl) btnURL += '&bl=' + encodeURIComponent(params.bl);
    if (params.fSid) btnURL += '&fSid=' + encodeURIComponent(params.fSid);
  ...
  var highlight = function(params){
    ...
    if (cpc && typeof cpc === 'object') cpc = cpc.value;
    if (typeof cpc === 'number') cpc = cpc;
    else if (cpc) cpc = parseFloat(cpc.replace(/[^\d,.]/g, '').replace(',', '.'));
  ...
  var renderWidgetTable = function(rows, params){
    ...
    if (params.keywordsPerPage) keywordsPerPage = params.keywordsPerPage;
    var pur = _credits > 0 ? 0 : 1;
    var excludeCols = params.excludeCols || [];
    var hasCPCColumn = metricsList.cpc && excludeCols.indexOf('cpc') === -1;
    var hasCompColumn = metricsList.comp && excludeCols.indexOf('comp') === -1;
    var hasTrendColumn = metricsList.trend && excludeCols.indexOf('trend') === -1;
    ...
    if (json) {
      ...
      html.push(
        ...
        hasTrendColumn && metricsList.trend ? '<th>' + trendColumnName + '</th>' : '',
        hasUsageColumn ? `<th>${params.usageColumnName ? params.usageColumnName : 'Usage'}</th>` : ''
      );
    }
    else {
      ...
      if (hasUsageColumn) {
        thHTML += `<th>${params.usageColumnName ? params.usageColumnName : 'Usage'}</th>`;
      }
      html.push(thHTML);
    }
  ...
  var renderIframeHTML = function(params){
    ...
    if (params.darkMode) iframeSrc += '&darkmode=' + params.darkMode;
    if (params.url) iframeSrc += '&url=' + encodeURIComponent(params.url);
    var html = '<div class="xt-widget-iframe"><iframe src="' + iframeSrc + '" scrolling="no"></iframe></div>';
    return html;
  };
  ...
  const renderGenericWidget = function(params){
    ...
    let countryTitle = country ? `(${country})` : '';
    let rootClassName = '';
    if (params.rootClassName) rootClassName = params.rootClassName;
    if (!$root[0]) {
      $root = $(rootTagName, {
        id: rootId,
        class: `xt-ke-card xt-widget-generic ${rootClassName}`
      });
      let $addTo = $(params.parentSelector);
      $root[addMethod]($addTo);
      if (params.onAdded) params.onAdded($root);
