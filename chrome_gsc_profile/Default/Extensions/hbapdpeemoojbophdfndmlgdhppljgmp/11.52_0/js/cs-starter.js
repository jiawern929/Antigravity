var Starter = (function(){
  ...
  var checkURL = function( sources, plan, config ){
    ...
    var sourceSec;
    if (typeof ToolSec !== 'undefined') {
      sourceSec = ToolSec.getSource();
    }
    ...
    if (sources[source]) {
      ...
      if (hasCredits) {
        ...
        var freeSources = 'bingco youtub gtrend instgr ebayco etsyco amazon gplsto pntrst openai gemini claude deepsk xtwttr gwmtoo reddit ycmbnt quoraa stacko';
        // config.areSubsEnabled = true;
        if (config.areSubsEnabled) freeSources = 'instgr openai gemini claude deepsk pntrst youtub bingco duckgo xtwttr gwmtoo reddit ycmbnt quoraa stacko';
        if ((source === 'gsearc' && isGSearchURL()) || freeSources.indexOf(source) !== -1) {
          Tool.init();
          if (source === 'openai' || source === 'gemini' || source === 'claude' || source === 'deepsk') addSettingsButton();
        }
      }
    }
  };
  ...
  var addSettingsButton = function(){
    ...
    if (settings.showAddAllButton) {
      if ('amazon ebayco youtub instgr yahoo openai gemini claude deepsk xtwttr reddit ycmbnt quoraa stacko'.indexOf(source) === -1) {
    ...
    if (settings.showChatGPTactions) {
      if (source === 'openai' || source === 'gemini' || source === 'claude' || source === 'deepsk') {
