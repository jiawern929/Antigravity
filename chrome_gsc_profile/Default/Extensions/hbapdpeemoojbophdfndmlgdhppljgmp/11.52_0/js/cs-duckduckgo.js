var Tool = (function(){
  ...
  var initSuggestions = function(){
    var timer = setInterval(function(){
      if (!observer) {
        var node = $('.search__autocomplete, form[role=search], #listbox--searchbox_homepage')[0];
        if (node) {
          clearInterval(timer);
  ...
  var getQuery = function(){
    var res = $('#search_form_input').val();
    if (!res) res = $('#searchbox_input').val();
    return res;
  };
  ...
  var processSuggestionsListResponse = function(json, keywords){
    ...
    Common.addCopyExportButtons($('.search__autocomplete, #listbox--searchbox_homepage'), data, filename);
  };
